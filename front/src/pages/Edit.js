...
function Edit() {
  ...
  useEffect(()=>{
    axios.get(`http://localhost:3001/pessoas/porCPF/${CPF_PK}`).then((response) => {
  ...
  const valoresIniciais = {
    ...
    const { CPF_PK } = this.props.match.params;
    const response = await api.get(`/pessoas/porCPF/${CPF_PK}`);
    this.setState({
        CPF_PK: response.data.CPF_PK,
        nome_pessoa: response.data.nome_pessoa,
        telefone_pessoa: response.data.telefone_pessoa,
        dt_nasc_pessoa: response.data.dt_nasc_pessoa,
    })
